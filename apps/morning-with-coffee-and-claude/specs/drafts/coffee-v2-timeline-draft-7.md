...
export async function saveBriefing(briefing: {
  ...
  await db.execute({
    sql: `
      INSERT INTO briefings (slot, date, run_at, tldr_facts, tldr_try_today, tldr_insight, item_ids, item_count, sentiment_snapshot)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
4. Fetch changelog and ecosystem data from their respective tables (these are daily/global, not per-briefing)

Return `TimelineData` with all briefings hydrated.

**Important**: Items are fetched by their stored IDs, NOT re-derived from source/date filtering. This ensures historical briefings always show the exact content that was present when they ran, even if items are later pruned.
...
**2g. Extend `runPipelineTransaction()` (Option A from Task 4h):**

Add an optional `briefingInput` parameter to the existing `runPipelineTransaction()`. When provided, insert/upsert the briefings row within the same transaction that writes items + sentiment. This ensures atomicity — items and their briefing row are always consistent.
...
**4f. Update idempotency check (mutex guard)**: Replace the existing `getSentimentSnapshot(today)` check with a new `getBriefing(slot, today)` query against the **briefings** table: `SELECT 1 FROM briefings WHERE slot = ? AND date = ?`. If it exists and `?force=true` is not set, return early. This is critical — the old check uses `sentiment_daily` which has a single row per date; with 3 slots/day, the midday and evening runs would incorrectly skip.
...
**4h. Atomic briefing write**: After classification, save items + sentiment + briefing in one operation. Two options:
- **Option A (recommended)**: Extend `runPipelineTransaction()` to accept an optional briefing input. If provided, save the briefing row within the same transaction. This ensures items and their briefing row are always consistent.
- **Option B**: Call `saveBriefing()` separately after `runPipelineTransaction()`. Simpler but non-atomic — if saveBriefing fails, items are saved but the briefing reference is lost.
The pipeline-worker should implement Option A.

**4i. Track fetcher status**: For each fetcher in the slot, record success/failure in a `fetcherStatus` object (e.g., `{"reddit":"ok","twitter":"error:timeout"}`). Pass this to the briefing save. The UI can optionally show a subtle indicator if a source failed.

**4j. TL;DR is best-effort**: If `generateBriefingTldr()` fails (Haiku rate limit, timeout, outage), still create the briefing with the fetched items. Store `null` for tldr fields. The `TldrSection` component handles null gracefully (shows items without summary). Items are always more valuable than summaries.
...
3. **`briefings` table** exists in Turso schema with columns: id, slot, date, run_at, tldr_facts, tldr_try_today, tldr_insight, item_ids, item_count, sentiment_snapshot, created_at
...
19. **Idempotency check** queries the briefings table (not sentiment_daily)
20. **sentiment_daily** is only written during the evening slot; morning/midday store per-slot sentiment in briefings.sentiment_snapshot
21. **Items + briefing saved atomically** via extended `runPipelineTransaction()`
22. **Ecosystem fetchers** (Awesome Lists, Agent Configs) only run during midday slot
23. **Missing item IDs** handled gracefully — `getItemsByIds()` skips deleted items
24. **First deployment** with zero briefings falls back to legacy DashboardLayout
