...
- Use the `renderMarkdown()` function from PulseSummary for link rendering (extract it to a shared util or duplicate)
...
Update imports accordingly. Keep the `revalidate = 86400` or consider changing to a shorter revalidation window since content updates 3x/day — `revalidate = 21600` (6 hours) would ensure fresh content between briefings.
...
- **Future Enhancements**: New sources (Discord, Anthropic docs changelog), repo intelligence (skill gaps, similar projects), audio briefings. These are tracked in `specs/coffee-v2-plan.md` as Priorities 3-4.
