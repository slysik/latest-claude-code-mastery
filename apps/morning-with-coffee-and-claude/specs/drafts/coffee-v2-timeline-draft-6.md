...
Query the `briefings` table for the last N days, ordered by date DESC then slot order (evening > midday > morning). For each briefing row:
1. Parse `item_ids` JSON array from the row
2. Batch-fetch items using `SELECT * FROM items WHERE id IN (...)`
...
**2d. Add `getUpcomingSlots(date)` helper:**

For the current day, determine which slots haven't run yet by checking which (slot, date) combinations are missing from the briefings table. Return an array of `BriefingSlot` values for slots that are expected but haven't run. Used by the UI to show "coming at X PM" placeholders.
...
**4f. Update idempotency check (mutex guard)**: At the very top of the handler (after auth), check if a briefing for this slot+date already exists via `SELECT 1 FROM briefings WHERE slot = ? AND date = ?`. If it exists and `?force=true` is not set, return early with `{ message: "Briefing already exists", slot, date }`. This prevents race conditions if the previous cron run is still executing when the next one fires.

**4g. Track fetcher status**: For each fetcher in the slot, record success/failure in a `fetcherStatus` object (e.g., `{"reddit":"ok","twitter":"error:timeout"}`). Pass this to `saveBriefing()` for storage. The UI can optionally show a subtle indicator if a source failed.

**4h. TL;DR is best-effort**: If `generateBriefingTldr()` fails (Haiku rate limit, timeout, outage), still create the briefing with the fetched items. Store `null` for tldr fields. The `TldrSection` component handles null gracefully (shows items without summary). Items are always more valuable than summaries.
